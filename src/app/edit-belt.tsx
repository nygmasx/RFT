import { router } from 'expo-router';
import { useMemo, useState } from 'react';
import {
  Pressable, ScrollView, StyleSheet, Text, TextInput, View,
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';

import { FONTS, Theme } from '@/constants/theme';
import { useTheme } from '@/context/ThemeContext';
import { MY_BELT } from '@/data/rft-data';

type BeltColor = 'blanche' | 'bleue' | 'violette' | 'marron' | 'noire';

const BELT_COLORS: Record<BeltColor, string> = {
  blanche:  '#EFE7D2',
  bleue:    '#1E4B86',
  violette: '#4D2D74',
  marron:   '#4A2E1C',
  noire:    '#0A0A0A',
};

const BELT_ORDER: BeltColor[] = ['blanche', 'bleue', 'violette', 'marron', 'noire'];

const BELT_LABEL: Record<BeltColor, string> = {
  blanche:  'BLANCHE',
  bleue:    'BLEUE',
  violette: 'VIOLETTE',
  marron:   'MARRON',
  noire:    'NOIRE',
};

function BJJBelt({ color = 'marron', stripes = 2, height = 32 }: { color?: BeltColor | string; stripes?: number; height?: number }) {
  const key = color as BeltColor;
  const bg = BELT_COLORS[key] ?? BELT_COLORS.marron;
  const tapeColor = color === 'noire' ? '#9B1B14' : '#0A0A0A';
  return (
    <View style={[beltS.belt, { height, backgroundColor: bg }]}>
      <View style={beltS.stitch} />
      <View style={[beltS.tape, { backgroundColor: tapeColor }]}>
        {[0, 1, 2, 3].map((i) => (
          <View key={i} style={[beltS.stripe, { backgroundColor: i < stripes ? '#F2EDE0' : 'rgba(255,255,255,0.05)' }]} />
        ))}
      </View>
    </View>
  );
}

const beltS = StyleSheet.create({
  belt: {
    borderRadius: 2, borderWidth: 1, borderColor: 'rgba(0,0,0,0.55)',
    flexDirection: 'row', overflow: 'hidden',
  },
  stitch: {
    position: 'absolute', left: 6, right: 90, top: '50%', height: 1,
    backgroundColor: 'rgba(255,255,255,0.12)',
  },
  tape: {
    marginLeft: 'auto', width: 80, borderLeftWidth: 1, borderLeftColor: 'rgba(0,0,0,0.8)',
    flexDirection: 'row', paddingHorizontal: 8, gap: 4, alignItems: 'stretch',
  },
  stripe: { flex: 1 },
});

export default function EditBeltScreen() {
  const { theme: t } = useTheme();
  const styles = useMemo(() => makeStyles(t), [t]);

  const [selectedColor, setSelectedColor] = useState<BeltColor>(MY_BELT.color);
  const [selectedStripes, setSelectedStripes] = useState(MY_BELT.stripes);
  const [promotedBy, setPromotedBy] = useState(MY_BELT.promotedBy);
  const [promotedDate, setPromotedDate] = useState(MY_BELT.promotedDate);

  return (
    <View style={styles.container}>
      <SafeAreaView edges={['top']}>
        <View style={styles.header}>
          <Pressable style={styles.backBtn} onPress={() => router.back()}>
            <Text style={styles.backIcon}>‹</Text>
          </Pressable>
          <Text style={styles.title}>MON GRADE</Text>
          <Pressable onPress={() => router.back()}>
            <Text style={styles.saveText}>ENREGISTRER</Text>
          </Pressable>
        </View>
      </SafeAreaView>

      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>

        {/* Belt color picker */}
        <Text style={styles.sectionLabel}>COULEUR DE CEINTURE</Text>
        <View style={styles.card}>
          <View style={styles.colorRow}>
            {BELT_ORDER.map((b) => {
              const active = b === selectedColor;
              return (
                <Pressable key={b} style={styles.colorCell} onPress={() => setSelectedColor(b)}>
                  <View style={[styles.colorSwatch, {
                    backgroundColor: BELT_COLORS[b],
                    borderWidth: active ? 2 : 1,
                    borderColor: active ? t.crimson : 'rgba(0,0,0,0.55)',
                  }]} />
                  <Text style={[styles.colorLabel, { color: active ? t.crimson : t.textMute }]}>
                    {BELT_LABEL[b].slice(0, 3).toUpperCase()}.
                  </Text>
                  {active && <View style={styles.colorDot} />}
                </Pressable>
              );
            })}
          </View>
        </View>

        {/* Live preview */}
        <Text style={styles.sectionLabel}>APERÇU</Text>
        <View style={styles.card}>
          <BJJBelt color={selectedColor} stripes={selectedStripes} height={48} />
          <View style={styles.previewRow}>
            <Text style={styles.previewName}>
              {BELT_LABEL[selectedColor]} · <Text style={{ color: t.crimson }}>{selectedStripes} BARRES</Text>
            </Text>
          </View>
        </View>

        {/* Stripe selector */}
        <Text style={styles.sectionLabel}>NOMBRE DE BARRES</Text>
        <View style={styles.card}>
          <View style={styles.stripeRow}>
            {[0, 1, 2, 3, 4].map((n) => (
              <Pressable
                key={n}
                style={[styles.stripePill, selectedStripes === n && styles.stripePillActive]}
                onPress={() => setSelectedStripes(n)}
              >
                <Text style={[styles.stripePillText, selectedStripes === n && styles.stripePillTextActive]}>
                  {n}
                </Text>
              </Pressable>
            ))}
          </View>
          <Text style={styles.stripeNote}>
            0-3 barres pour les ceintures de progression · 4 barres = prêt pour la promotion
          </Text>
        </View>

        {/* Progression path */}
        <Text style={styles.sectionLabel}>PROGRESSION BJJ</Text>
        <View style={styles.card}>
          <View style={styles.progressRow}>
            {BELT_ORDER.map((b, i) => {
              const active = b === selectedColor;
              return (
                <View key={b} style={styles.progressCell}>
                  {i > 0 && <View style={styles.progressLine} />}
                  <View style={[styles.progressSwatch, {
                    backgroundColor: BELT_COLORS[b],
                    width: active ? 42 : 30,
                    height: active ? 24 : 16,
                    borderWidth: active ? 2 : 1,
                    borderColor: active ? t.crimson : 'rgba(0,0,0,0.55)',
                  }]} />
                  <Text style={[styles.progressLabel, {
                    color: active ? t.crimson : t.textMute,
                    fontWeight: active ? '700' : '400',
                  }]}>
                    {BELT_LABEL[b].slice(0, 4)}
                  </Text>
                </View>
              );
            })}
          </View>
        </View>

        {/* Promotion fields */}
        <Text style={styles.sectionLabel}>DÉTAILS</Text>
        <View style={styles.card}>
          <View style={styles.fieldRow}>
            <Text style={styles.fieldLabel}>PROMU PAR</Text>
            <TextInput
              style={styles.input}
              value={promotedBy}
              onChangeText={setPromotedBy}
              placeholderTextColor={t.textMute}
              selectionColor={t.crimson}
            />
          </View>
          <View style={styles.divider} />
          <View style={styles.fieldRow}>
            <Text style={styles.fieldLabel}>DATE DE PROMOTION</Text>
            <TextInput
              style={styles.input}
              value={promotedDate}
              onChangeText={setPromotedDate}
              placeholder="jj.mm.aaaa"
              placeholderTextColor={t.textMute}
              selectionColor={t.crimson}
            />
          </View>
        </View>

        <View style={{ height: 40 }} />
      </ScrollView>
    </View>
  );
}

function makeStyles(t: Theme) {
  return StyleSheet.create({
    container: { flex: 1, backgroundColor: t.ink },
    header: {
      flexDirection: 'row', alignItems: 'center', gap: 12,
      paddingHorizontal: 18, paddingBottom: 14, paddingTop: 4,
      borderBottomWidth: 1, borderBottomColor: t.hairline,
    },
    backBtn: { padding: 4 },
    backIcon: { fontSize: 28, color: t.bone, lineHeight: 28 },
    title: {
      flex: 1, fontFamily: FONTS.display, fontSize: 16, color: t.bone,
      fontWeight: '900', letterSpacing: 0.5,
    },
    saveText: { fontFamily: FONTS.mono, fontSize: 11, color: t.crimson, fontWeight: '700', letterSpacing: 1 },
    scroll: { paddingHorizontal: 20, paddingTop: 16, gap: 8 },

    sectionLabel: {
      fontFamily: FONTS.mono, fontSize: 9.5, color: t.textMute, letterSpacing: 2,
      marginTop: 8, marginBottom: 4,
    },
    card: {
      backgroundColor: t.surface, borderWidth: 1, borderColor: t.hairline,
      borderRadius: 3, padding: 14,
    },

    // Color picker
    colorRow: { flexDirection: 'row', gap: 8 },
    colorCell: { flex: 1, alignItems: 'center', gap: 4 },
    colorSwatch: { width: '100%', height: 36, borderRadius: 2 },
    colorLabel: { fontFamily: FONTS.mono, fontSize: 9 },
    colorDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: '#C8362D' },

    // Preview
    previewRow: { marginTop: 12, alignItems: 'center' },
    previewName: {
      fontFamily: FONTS.display, fontSize: 22, color: t.bone, fontWeight: '900', letterSpacing: 0.5,
    },

    // Stripe selector
    stripeRow: { flexDirection: 'row', gap: 8 },
    stripePill: {
      flex: 1, paddingVertical: 10, borderRadius: 2, borderWidth: 1,
      borderColor: t.hairline, alignItems: 'center', backgroundColor: t.surface,
    },
    stripePillActive: { backgroundColor: t.crimson, borderColor: t.crimson },
    stripePillText: { fontFamily: FONTS.display, fontSize: 16, color: t.textDim, fontWeight: '700' },
    stripePillTextActive: { color: t.bone },
    stripeNote: {
      fontFamily: FONTS.mono, fontSize: 9, color: t.textMute, letterSpacing: 0.5,
      marginTop: 10, lineHeight: 14,
    },

    // Progression
    progressRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
    progressCell: { alignItems: 'center', gap: 4, flexDirection: 'column', flex: 1 },
    progressLine: {
      position: 'absolute', left: 0, right: '50%', top: '30%',
      height: 1, backgroundColor: t.hairlineStrong,
    },
    progressSwatch: { borderRadius: 2 },
    progressLabel: { fontFamily: FONTS.mono, fontSize: 8, letterSpacing: 0.5 },

    // Fields
    fieldRow: { paddingVertical: 10 },
    fieldLabel: { fontFamily: FONTS.mono, fontSize: 9.5, color: t.textMute, letterSpacing: 1.5, marginBottom: 6 },
    input: { fontFamily: FONTS.body, fontSize: 15, color: t.bone, fontWeight: '500', paddingVertical: 0 },
    divider: { height: 1, backgroundColor: t.hairline, marginVertical: 2 },
  });
}
